            ...
            for (int i = 0; i < 3; i++) {
                ...
                for (int l = 0; l < 3; l++) {
                    ...
                    for (int j = 0; j < numberAnchorSensors[m]; j++) {
                        anchorSensors.add(new Sensor(
                                new Position(random.nextInt(areaSize), random.nextInt(areaSize), random.nextInt(areaSize)),
                    ...
                    for (int j = 0; j < numOfRandomSensors; j++) {
                        ...
                        int z = random.nextInt(areaSize);
                        double generatedError = generateError(error);
                        sensors.add(new Sensor(new Position(x, y, z), 10, false, false, generatedError));
                    }
...
class Sensor implements Cloneable {
    ...
    protected Sensor clone() {
        ...
        if (predictedPosition != null) {
            newPredictedPosition = new Position(predictedPosition.x, predictedPosition.y, predictedPosition.z);
        }
        return new Sensor(
